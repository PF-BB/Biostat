...
\item Personnalisation délicate parfois (idem pour graphics et \pkg{ggplot2})
\end{itemize}
...
facettes pour représenter des distributions conditionnelles.

La notation utilisée est la notation par formule :
\begin{itemize}
\item y ~ x | a, y en fonction de x condit. à a
\item y ~ x | a + b, y en fonction de x condit. à a et b 
\end{itemize}
...
\begin{itemize}
\item Il est important de vérifier le codage des variables, et de recoder
  en fonction des besoins de l'analyse ou de la visualisation.
\item On caractérise d'abord les distributions univariées avant
...
\item L'interface \pkg{lattice} utilise la même notation par formule que les
